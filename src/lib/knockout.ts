...
export const R32_PAIRINGS: Array<[number, number]> = [
  [13, 14], [3, 25], [5, 26], [6, 15],
  [9, 27], [17, 21], [1, 28], [12, 29],
  [4, 30], [7, 31], [23, 24], [8, 22],
  [2, 32], [10, 20], [11, 16], [16, 19],
];
...
export async function getAllGroupResults(tournamentId: string): Promise<GroupResult[]> {
  ...
  const { data: stage } = await supabase
    ...
    .eq('tournament_id', tournamentId).eq('stage_type', 'groups').maybeSingle();
  if (!stage) return [];
  const { data: groups } = await supabase
    .from('groups').select('id, name').eq('stage_id', stage.id).order('position');
...
export function computeBestThirds(groupResults: GroupResult[]): BestThirdRow[] {
  ...
  thirds.forEach((t, i) => { t.rank = i + 1; t.qualifies = i < 8; });
  return thirds;
}
...
export async function generateBracket(tournamentId: string): Promise<{ ok: true } | { ok: false; reason: string }> {
  const supabase = createClient();
  if (await bracketExists(tournamentId)) return { ok: false, reason: 'Bracket already generated' };
  const groupResults = await getAllGroupResults(tournamentId);
  if (groupResults.length !== 12) return { ok: false, reason: `Expected 12 groups, got ${groupResults.length}` };
  if (groupResults.some(g => g.hasUnresolvedTie)) return { ok: false, reason: 'Resolve all group tiebreakers before generating bracket' };
  ...
  for (let i = 0; i < 12; i++) {
    ...
    seeds.push({ seed: i + 1, participantId: winner.participant_id, sourceLabel: `${g.label}1`, sourceGroup: g.label, sourcePosition: 1, isBestThird: false });
  }
  for (let i = 0; i < 12; i++) {
    const g = groupResults[i];
    const second = g.standings[1];
    if (!second) return { ok: false, reason: `Group ${g.label} has no runner-up` };
    seeds.push({ seed: i + 13, participantId: second.participant_id, sourceLabel: `${g.label}2`, sourceGroup: g.label, sourcePosition: 2, isBestThird: false });
  }
  const bestThirds = computeBestThirds(groupResults).filter(t => t.qualifies);
  if (bestThirds.length !== 8) return { ok: false, reason: `Expected 8 qualifying thirds, got ${bestThirds.length}` };
  bestThirds.forEach((bt, i) => {
    seeds.push({ seed: 25 + i, participantId: bt.participant_id, sourceLabel: `BT${i + 1}`, sourceGroup: bt.group_label, sourcePosition: 3, isBestThird: true });
  });
  ...
  const { data: stage, error: stageErr } = await supabase
    .from('stages')
    .insert({ tournament_id: tournamentId, stage_number: 2, stage_type: 'single_elimination', status: 'in_progress' })
    .select('id').single();
  ...
  for (let i = 0; i < R32_PAIRINGS.length; i++) {
    ...
    if (!homeId || !awayId) return { ok: false, reason: `Missing participant for seed ${seedA} or ${seedB}` };
  ...
  const placeholderRounds = [{ round: 2, count: 8 }, { round: 3, count: 4 }, { round: 4, count: 2 }, { round: 5, count: 1 }];
...
async function createFeeders(tournamentId: string, stageId: string): Promise<void> {
  ...
  const { data: matches } = await supabase
    ...
    .eq('stage_id', stageId)
    .order('round').order('match_number_in_round');
  if (!matches) return;
  const byRoundAndNum = new Map<string, string>();
  for (const m of matches) byRoundAndNum.set(`${m.round}-${m.match_number_in_round}`, m.id);
  const transitions = [{ fromRound: 1, toRound: 2 }, { fromRound: 2, toRound: 3 }, { fromRound: 3, toRound: 4 }, { fromRound: 4, toRound: 5 }];
...
export async function getBracketView(tournamentId: string) {
  ...
  const { data: stage } = await supabase
    .from('stages').select('id')
    .eq('tournament_id', tournamentId).eq('stage_type', 'single_elimination').maybeSingle();
